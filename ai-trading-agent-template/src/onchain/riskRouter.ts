...
export interface RiskTradeRecord {
  count: bigint;
  windowStart: bigint;
}

function normalizeRiskValidationResult(result: unknown): { approved: boolean; reason: string } {
  ...
  return { approved: false, reason: "RiskRouter returned no validation result" };
}

function extractRiskRouterErrorMessage(error: unknown): string {
  ...
  if (error && typeof error === "object") {
    ...
    if (typeof record.shortMessage === "string" && record.shortMessage.trim()) {
      return record.shortMessage.trim();
    }
    if (typeof record.message === "string" && record.message.trim()) {
      return record.message.trim();
    }
    if (typeof record.info?.error?.message === "string" && record.info.error.message.trim()) {
      return record.info.error.message.trim();
    }
  }

  return "RiskRouter rejected the intent";
}
...
export class RiskRouterClient {
   ...
   */
  async signIntent(intent: TradeIntent, agentWalletSigner: ethers.Signer): Promise<SignedTradeIntent> {
    ...
    const intentHash = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(
      ...
      [intent.agentId, intent.agentWallet, intent.pair, intent.action, intent.amountUsdScaled, intent.nonce, intent.deadline]
    ));
  ...
  async submitIntent(signed: SignedTradeIntent): Promise<RiskValidationResult> {
    ...
    try {
      const simulation = normalizeRiskValidationResult(
        await this.contract.submitTradeIntent.staticCall(intentStruct, signed.signature)
      );

      if (!simulation.approved) {
        return {
          approved: false,
          reason: simulation.reason || "RiskRouter rejected the intent",
      ...
      return {
        approved: false,
        reason: extractRiskRouterErrorMessage(error),
        intentHash: signed.intentHash,
      };
    }
  }
  ...
  async simulateIntent(intent: TradeIntent): Promise<RiskValidationResult> {
    ...
    return normalizeRiskValidationResult(result);
  }
  ...
  async getRiskParams(agentId: bigint) {
    const p = await this.contract.riskParams(agentId);
    return {
  ...
  async getTradeRecord(agentId: bigint): Promise<RiskTradeRecord> {
    const record = await this.contract.getTradeRecord(agentId);
    return {
      count: BigInt(record.count),
      windowStart: BigInt(record.windowStart),
    };
  }
