...
```bash
git clone https://github.com/HyperionBurn/beta_tradingagent.git
...
[`scripts/register-agent.ts` L47](../scripts/register-agent.ts#L47) calls [`src/agent/identity.ts` L46–L68](../src/agent/identity.ts#L46-L68):
...
Which calls `register()` on the contract ([`contracts/AgentRegistry.sol` L92–L120](../contracts/AgentRegistry.sol#L92-L120)):
