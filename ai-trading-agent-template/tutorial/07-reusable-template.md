...
Implement [`TradingStrategy`](../src/agent/strategy.ts) directly:
...
Then in [`src/agent/index.ts` L28 & L211](../src/agent/index.ts#L28), swap the strategy import:
...
Full stub in [`src/agent/strategy.ts` L90](../src/agent/strategy.ts#L90) — uncomment and fill in your client:
...
See also [`src/agent/strategy.ts` L27](../src/agent/strategy.ts#L27) for the `MomentumStrategy` reference implementation:
...
|-------|--------------|-----|
| Trading strategy / model | ✅ Yes | Implement [`TradingStrategy`](../src/agent/strategy.ts#L27) |
| Trading pair | ✅ Yes | [`TRADING_PAIR`](../src/agent/index.ts#L41) in `.env` |
| Poll interval | ✅ Yes | [`POLL_INTERVAL_MS`](../src/agent/index.ts#L42) in `.env` |
| Risk parameters | ✅ Yes | [`setRiskParams()`](../src/onchain/riskRouter.ts#L183) call |
| Agent metadata | ✅ Yes | Edit name/description/capabilities in [`register-agent.ts`](../scripts/register-agent.ts#L48) |
| ERC-8004 identity scheme | ❌ Fixed | Same for all agents |
| Vault + RiskRouter contracts | ❌ Fixed | Same contracts, per-agent config |
| Kraken API client | ❌ Fixed | [`src/exchange/kraken.ts`](../src/exchange/kraken.ts) |
| EIP-712 checkpoint format | ❌ Fixed | [`src/explainability/checkpoint.ts`](../src/explainability/checkpoint.ts#L28) |

---

## Checklist for a new team

1. `git clone https://github.com/HyperionBurn/beta_tradingagent.git && cd ai-trading-agent-template`
2. `npm install`
...
3. Set sensible risk params via [`setRiskParams()`](../src/onchain/riskRouter.ts#L183)
...
The project ships with a live web dashboard ([`scripts/dashboard.ts`](../scripts/dashboard.ts)) that reads from `checkpoints.jsonl` and auto-refreshes every 5 seconds:
