...
[`src/explainability/checkpoint.ts` L28–L41](../src/explainability/checkpoint.ts#L28-L41) defines the typed structure:
...
- Strings can be arbitrarily long: hashing keeps the signed payload compact ([`checkpoint.ts` L69](../src/explainability/checkpoint.ts#L69))
- The hash is a commitment: if you have the hash and the original string, you can verify they match ([`verifyReasoningIntegrity()`](../src/explainability/checkpoint.ts#L145))
- The full reasoning string is stored alongside the checkpoint in `checkpoints.jsonl` ([`index.ts` L193](../src/agent/index.ts#L193))

---

## Generating a checkpoint

[`src/explainability/checkpoint.ts` L59–L67](../src/explainability/checkpoint.ts#L59-L67):
