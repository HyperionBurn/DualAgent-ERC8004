...
Every `TradeDecision` returned by your strategy must include a `reasoning` string ([`src/types/index.ts` L30–L37](../src/types/index.ts#L30-L37)):
...
The reasoning field flows through three places automatically, once per tick (every 1 minute by default):

| Where | What you see | Code reference |
|-------|-------------|----------------|
| **Terminal** | Printed immediately after each decision via `formatExplanation()` | [`src/agent/index.ts` L119](../src/agent/index.ts#L119) |
| **Terminal (checkpoint block)** | Printed again inside the signed checkpoint summary via `formatCheckpointLog()` | [`src/agent/index.ts` L174](../src/agent/index.ts#L174) |
| **EIP-712 signature** | `keccak256(reasoning)` is computed and included in the signed payload | [`src/explainability/checkpoint.ts` L69](../src/explainability/checkpoint.ts#L69) |
| **`checkpoints.jsonl`** | Full reasoning string appended as JSON after every tick | [`src/agent/index.ts` L193](../src/agent/index.ts#L193) |
| **Dashboard** | Shown on each checkpoint card under the action badge | `http://localhost:3000` (poll every 5s) |

You'll see the first output in the terminal **within about 1 minute** of starting `npm run run-agent`. The default interval is set in [`src/agent/index.ts` L42](../src/agent/index.ts#L42) and can be overridden with `POLL_INTERVAL_MS` in `.env`.

---

## The `formatExplanation()` function

[`src/explainability/reasoner.ts` L19–L51](../src/explainability/reasoner.ts#L19-L51) wraps the decision + market context into a structured log line:
...
The demo runs `MomentumStrategy` — no LLM involved. Reasoning is generated directly from price arithmetic ([`src/agent/strategy.ts` L61–L72](../src/agent/strategy.ts#L61-L72)). That's intentional: the template works out of the box without any API keys beyond Kraken.

When you're ready to replace the strategy with a model, the codebase includes a ready-to-wire `LLMStrategy` stub at [`src/agent/strategy.ts` L90–L135](../src/agent/strategy.ts#L90-L135):
...
[`src/explainability/reasoner.ts` L56–L70](../src/explainability/reasoner.ts#L56-L70) — when a checkpoint is generated, `formatCheckpointLog()` produces a structured summary for the terminal:
...
1. [`kraken.getTicker()`](../src/agent/index.ts#L112) — fetch live price from Kraken
2. [`strategy.analyze(market)`](../src/agent/index.ts#L116) — strategy returns a `TradeDecision` (including `reasoning`)
3. [`formatExplanation()`](../src/agent/index.ts#L119) — prints the human-readable summary
4. [`generateCheckpoint()`](../src/agent/index.ts#L164) — signs the decision with EIP-712; reasoning is hashed into the signature
5. [`formatCheckpointLog()`](../src/agent/index.ts#L174) — prints the signed checkpoint block
6. [ValidationRegistry post](../src/agent/index.ts#L186) — checkpoint hash submitted on-chain to Sepolia
7. [`fs.appendFileSync()`](../src/agent/index.ts#L193) — checkpoint appended to `checkpoints.jsonl`
...
- Verify any signature with [`verifyCheckpoint()`](../src/explainability/checkpoint.ts#L116)
- Check that reasoning strings weren't tampered with using [`verifyReasoningIntegrity()`](../src/explainability/checkpoint.ts#L145)
