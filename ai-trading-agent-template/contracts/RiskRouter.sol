...
contract RiskRouter is EIP712 {
    ...
    struct DrawdownState {
        uint256 peakEquityUsdScaled;
        uint256 currentEquityUsdScaled;
        uint256 drawdownBps;
        uint256 lastUpdated;
        bool    circuitBreaker;
    }
    ...
    mapping(uint256 => DrawdownState) private _drawdownStates;
    ...
    event RiskParamsSet(uint256 indexed agentId, uint256 maxPositionUsdScaled, uint256 maxDrawdownBps, uint256 maxTradesPerHour, bool active);
    event DrawdownStateUpdated(
        uint256 indexed agentId,
        uint256 currentEquityUsdScaled,
        uint256 peakEquityUsdScaled,
        uint256 drawdownBps,
    ...
    function setRiskParams(
        ...
        emit RiskParamsSet(agentId, maxPositionUsdScaled, maxDrawdownBps, maxTradesPerHour, true);
    }

    /**
     * @notice Report latest equity for drawdown tracking.
     * @dev Restricted to owner/orchestrator so score inputs are controlled.
     */
    function reportEquity(
        uint256 agentId,
        uint256 equityUsdScaled,
        bytes32 outcomeRef
    ) external onlyOwner {
        require(agentRegistry.isRegistered(agentId), "RiskRouter: agent not registered");
        require(outcomeRef != bytes32(0), "RiskRouter: outcomeRef required");

        DrawdownState storage state = _drawdownStates[agentId];
        if (state.peakEquityUsdScaled == 0 || equityUsdScaled > state.peakEquityUsdScaled) {
            state.peakEquityUsdScaled = equityUsdScaled;
        }

        state.currentEquityUsdScaled = equityUsdScaled;
        uint256 drawdownBps = 0;
        if (state.peakEquityUsdScaled > 0 && equityUsdScaled < state.peakEquityUsdScaled) {
            drawdownBps = ((state.peakEquityUsdScaled - equityUsdScaled) * 10000) / state.peakEquityUsdScaled;
        }

        state.drawdownBps = drawdownBps;
        state.lastUpdated = block.timestamp;

        RiskParams storage params = riskParams[agentId];
        state.circuitBreaker = params.active && params.maxDrawdownBps > 0 && drawdownBps > params.maxDrawdownBps;

        emit DrawdownStateUpdated(
            agentId,
            state.currentEquityUsdScaled,
            state.peakEquityUsdScaled,
            state.drawdownBps,
    ...
    function submitTradeIntent(
        ...
        if (!SignatureChecker.isValidSignatureNow(reg.agentWallet, digest, signature)) {
    ...
    function _validateRisk(
        ...
        if (!params.active) {
            ...
            DrawdownState storage drawdown = _drawdownStates[agentId];
            if (drawdown.circuitBreaker) return (false, "Drawdown circuit breaker active");
            if (
                params.maxDrawdownBps > 0
                && drawdown.lastUpdated > 0
                && drawdown.drawdownBps > params.maxDrawdownBps
            ) {
                return (false, "Exceeds maxDrawdownBps");
            }
    ...
    function getRiskParams(uint256 agentId) external view returns (RiskParams memory) {
        return riskParams[agentId];
    }

    function getDrawdownState(uint256 agentId) external view returns (DrawdownState memory) {
        return _drawdownStates[agentId];
    }
